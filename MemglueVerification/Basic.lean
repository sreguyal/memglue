...
-- instance : Inhabited (SystemConfig) where default := {threads := 2, steps := 2, addrCount := 2}
...
-- abbrev QCnt (c : SystemConfig) : Type := Fin (c.steps + 1)
-- instance (c : SystemConfig) : Inhabited (QInd c) where default := Fin.ofNat (c.steps) 0
-- instance (c : SystemConfig) : Inhabited (QCnt c) where default := Fin.ofNat (c.steps + 1) c.steps
-- #eval (default : QInd default)
-- #eval (default : QCnt default)
...
-- def outputToList {c : SystemConfig} (o : Output c) : List (List (Option Nat)) :=
--   (List.finRange c.threads.val).map fun t =>
--     (List.finRange c.steps.val).map fun s => o t s
-- instance {c : SystemConfig} [Repr (Option Nat)] : Repr (Output c) where
--   reprPrec o _ :=
--     repr (outputToList o)
-- -- #check Output 3 2
...
-- def testOut := updateVal (1 : ShimId (default)) 10 (default : Execution (default)) ((default : ShimType (default)).set 1 {(default : Shim (default)) with qInd := (1 : QInd (default)), qCnt := (1 : QCnt default)}) (default : Output (default))
-- #eval testOut
-- def testNoChange := updateVal (1 : ShimId (default)) 10 (default : Execution (default)) ((default : ShimType (default)).set 1 {(default : Shim (default)) with qInd := (1 : QInd (default)), qCnt := (0 : QCnt default)}) (default : Output (default))
-- #eval testNoChange
...
--     let (shimVec', e', o') := shimReceive msg state.shimVec state.execution state.output
--     let state' := {state with shimVec := shimVec', execution := e', output := o'}
...
--     if instr.access = PermissionType.load then
--         let (shimVec', net', e'', o') := shimRead shim instr.addr instr.stren state'.shimVec state'.net state'.execution state'.output
--         let state'' := {state' with shimVec := shimVec', net := net', execution := e'', output := o'}
--         state''
--     else
--         if instr.access = PermissionType.store then
...
--             state''
--         else
--             if instr.access = PermissionType.fence then
--                 let (shimVec', net') := shimFence shim state'.shimVec state'.net
--                 let state'' := {state' with shimVec := shimVec', net := net'}
--                 state''
--             else panic! "IssueInstr got instruction that wasn't load/store/fence"
