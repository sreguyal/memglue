...
inductive increment_init {c : SystemConfig} : IncState c → Prop where
    | IncInit :                     -- IncInit is a proof that the state {...} satisfies increment_init
        increment_init {
            cc := (default : CCMachine c)
            shimVec := (default : ShimType c)
            net := (default : NETOrdered c)
            execution := (default : Execution c)
...
example : increment_init starting_state := increment_init.IncInit
