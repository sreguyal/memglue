...
def updateVal {c : SystemConfig} (targetThread : ShimId c) (data : Data)
    ...
    if (targetStep.val < stepCount.val) ∧ ((e targetThread targetStep).access ≠ PermissionType.none) then
        ...
        -- keep same
        fun (t : ShimId c) => fun (s : Fin c.steps) => e t s
...
def testOut := updateVal (1 : ShimId (default)) 10 (default : Execution (default)) ((default : ShimType (default)).set 1 {(default : Shim (default)) with qInd := (1 : QInd (default)), qCnt := (2 : QCnt default)})
#eval testOut
def testNoChange := updateVal (1 : ShimId (default)) 10 (default : Execution (default)) ((default : ShimType (default)).set 1 {(default : Shim (default)) with qInd := (1 : QInd (default)), qCnt := (0 : QCnt default)})
#eval testNoChange
